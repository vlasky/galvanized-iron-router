...
const fireOnLinkClick = function (e) {
  const target = e.target instanceof Element ? e.target : e.target && e.target.parentElement;
  const link = target && target.closest(Location.options.linkSelector);

  if (!link)
    return;

  // Native delegated events keep currentTarget set to document. Preserve the
  // existing contract by setting it to the matched link during the callback.
  // Override it only for the synchronous callback so other document listeners
  // still observe the native value. A non-configurable event (e.g. a frozen
  // synthetic event) cannot carry the contract, so the click is left to the
  // browser rather than routed with a wrong currentTarget.
  const descriptor = Object.getOwnPropertyDescriptor(e, 'currentTarget');
  let overrode = false;
  ...
  try {
    ...
    if (overrode) {
      if (descriptor)
        Object.defineProperty(e, 'currentTarget', descriptor);
      else
...
/**
 * Throw a descriptive error for a linkSelector the browser cannot parse.
 * Element.closest throws on invalid selectors, which would otherwise surface
 * as an exception on every document click.
 */
const validateLinkSelector = function (selector) {
  try {
    document.documentElement.matches(selector);
  } catch (_err) {
    throw new Error(
      "Invalid linkSelector " + JSON.stringify(selector) + ": it must be a " +
      "valid CSS selector (jQuery-only extensions like ':visible' are not " +
      "supported).");
  }
};
...
Location.configure = function (options) {
  if (options && typeof options.linkSelector === 'string')
    validateLinkSelector(options.linkSelector);
...
Location.start = function () {
  ...
  validateLinkSelector(Location.options.linkSelector);
  ...
  window.addEventListener('popstate', setStateFromEventHandler);
  window.addEventListener('hashchange', setStateFromEventHandler);

  document.addEventListener('click', fireOnLinkClick);
...
Location.stop = function () {
  ...
  window.removeEventListener('popstate', setStateFromEventHandler);
  window.removeEventListener('hashchange', setStateFromEventHandler);
  document.removeEventListener('click', fireOnLinkClick);
