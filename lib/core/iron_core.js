...
Iron.utils.extend = function (Parent, props) {
  ...
  if (Object.prototype.hasOwnProperty.call(props, 'name') && typeof props.name === 'string' && props.name) {
    // Avoid relying on Function#name which is often non-configurable.
    try {
      Object.defineProperty(ctor, 'name', {value: props.name});
    } catch (_e) {
      // Ignore if name is non-configurable in this environment.
    }
    ctor._name = props.name;
  }
