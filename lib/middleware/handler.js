...
class Handler {
  ...
  /** @type {boolean} */
  middleware;
  /** @type {import('../router/route.js').Route} */
  route;
  ...
  constructor(path, fn, options) {
    ...
    if (options.name)
      ...
      this.name = path;
    else if (typeof fn === 'function' && fn.name)
      this.name = fn.name;
  ...
  test(path, options) {
    ...
    const isUrlMatch = this.compiledUrl.test(path);
   ...
   */
  clone() {
    const clone = new Handler(this.path, this.handle, this.options);
