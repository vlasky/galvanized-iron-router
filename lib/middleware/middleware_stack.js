 ...
 *
 */
const MiddlewareStack = function () {
  this._stack = [];
  this.length = 0;
...
MiddlewareStack.prototype.dispatch = function dispatch (url, context, done) {
  ...
  var wrappedDone = function () {
    ...
    var callback = done || originalNext;
    if (callback) {
      try {
  ...
  var next = Meteor.bindEnvironment(function boundNext (err) {
    ...
    if (handler.where !== where) {
      ...
      if (Meteor.isClient && handler.where === 'server' && self._onServerDispatch) {
        self._onServerDispatch.call(context, handler, url);
      }
      return next(err);
    }
