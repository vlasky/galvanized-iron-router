...
class DynamicTemplate {
  ...
  create(options) {
    ...
    _.each(['onViewCreated', 'onViewReady', '_onViewRendered', 'onViewDestroyed'], (hook) => {
      view[hook](() => {
  ...
  _attachEvents() {
    ...
    const attach = (range, element) => {
      _.each(boundMap, (/** @type {Function} */ handler, spec) => {
  ...
  _detachEvents() {
    _.each(this._eventHandles, (h) => { h.stop(); });
    this._eventHandles = [];
  }
  ...
  static _normalizeScopeBindings(bindings) {
    ...
    _.each(keys, (key) => {
      ...
      if (key === 'hash' && value && typeof value === 'object') {
        _.each(value, (hashValue, hashKey) => {
