...
class Url {
  ...
  constructor(url, options) {
    ...
    Object.assign(this, Url.parse(url));
  }
      ...
      path = path
        .replace(
          ...
          function (match, slash, format, key, capture, optional, offset) {
            ...
            value = typeof value === 'function' ? value.call(params) : value;
            const escapedValue = String(value).split('/').map((segment) => {
        ...
        .replace(
          ...
          function (match) {
            ...
            return paramValue.split('/').map((segment) => {
              return encodeURIComponent(segment);
            }).join('/');
          }
        );
