...
const debug = Iron.utils.debug('iron-router:RouteController');
...
RouteController.prototype._runRoute = function (route, url, done) {
  ...
  const subsList = this._collectHooks('subscriptions');
  subsList.forEach((subFunc) => {
    this.wait(subFunc.call(this));
  });
  ...
  const waitOnList = this._collectHooks('waitOn');

  waitOnList.forEach((waitOn) => {
    this.wait(waitOn.call(this));
  });

  // if we have a waitOn option, the loading hook will be
  // added at the end of the before hook stack, right before
  // the action function.
  const useLoadingHook = waitOnList.length > 0;
  ...
  const getMainTemplate = () => {
    return Tracker.nonreactive(() => {
      const region = this._layout._regions.main;
      return region && region.template();
    });
  };

  const prevLayout = getLayout();
  const prevMainTemplate = getMainTemplate();

  this.beginRendering((usedRegions) => {
    if (this.isStopped)
      return;

    const curLayout = getLayout();
    const curMainTemplate = getMainTemplate();
    ...
    const allRegions = this._layout.regionKeys();
    const unusedRegions = allRegions.filter(r => !usedRegions.includes(r));
    unusedRegions.forEach((r) => { this._layout.clear(r); });
  });

  const layoutData = this.lookupOption('data');
  this.layout(this.lookupOption('layoutTemplate'), {
    data: (typeof layoutData === 'undefined' && typeof this.data === 'function') ? this.data : layoutData
  });

  let stack = new MiddlewareStack;
  const onRunStack = new MiddlewareStack;
  const onRerunStack = new MiddlewareStack;
  ...
  if (useLoadingHook) {
    stack.push(Iron.Router.hooks.loading.bind(this));
  }

  const beforeHooks = this._collectHooks('onBeforeAction', 'before');
  ...
  Tracker.afterFlush(() => {
    Iron.utils.warn(this._rendered || this.isStopped,
...
RouteController.prototype.render = function (template, options) {
  ...
  if (arguments.length === 0) {
    const mainTemplate = this.lookupTemplate();
    // Pass the route controller's data context to the main template
    const dataOption = this.lookupOption('data');
    const data = (typeof dataOption === 'undefined' && typeof this.data === 'function') ? this.data : dataOption;
    const mainOptions = data ? { data: data } : {};
    const result = RouteController.__super__.render.call(this, mainTemplate, mainOptions);
    this.renderRegions();
    return result;
  } else {
    // Pass the route controller's data context to explicitly rendered templates
...
RouteController.prototype.renderRegions = function () {
  const regionTemplates = this.lookupRegionTemplates();
  ...
  //   }
  Object.keys(regionTemplates).forEach((templateName) => {
    this.render(templateName, regionTemplates[templateName]);
  });
};
...
RouteController.prototype.subscribe = function (/* same as Meteor.subscribe */) {
  const handle = Meteor.subscribe.apply(this, arguments);
...
if (Package.reload) {
  ...
  Package.reload.Reload._onMigrate('iron-router', () => { return [true, true] });

  // then when we come back up, check if it is set
  const data = Package.reload.Reload._migrationData('iron-router');
