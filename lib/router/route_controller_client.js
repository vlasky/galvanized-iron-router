...
RouteController.prototype._runRoute = function (route, url, done) {
  ...
  this.beginRendering((usedRegions) => {
    ...
    const unusedRegions = _.difference(allRegions, usedRegions);
    _.each(unusedRegions, (r) => { this._layout.clear(r); });
  });
  ...
  if (useLoadingHook) {
    stack.push(_.bind(Iron.Router.hooks.loading, this));
  }
...
RouteController.prototype.renderRegions = function () {
  ...
  //   }
  _.each(regionTemplates, (opts, templateName) => {
    this.render(templateName, opts);
  });
};
