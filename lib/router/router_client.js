...
Router.prototype.init = function (options) {
  ...
  // the current() dep
  self._currentDep = new Tracker.Dependency;
  ...
  controller.dispatch(self._stack, url, function onRouterDispatchCompleted (err) {
    if (err)
      ...
      if (!controller.isHandled()) {
        ...
        var state = Tracker.nonreactive(function () { return controller.location.get().options.historyState; });
...
Router.prototype.start = function () {
  ...
  self._locationComputation = Tracker.autorun(function onLocationChange (c) {
    var controller;
