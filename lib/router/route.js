...
class Route {
  ...
  constructor(path, fn, options) {
    // Clear the function's name so the Handler derives the route name from
    // the URL path instead of fn.name. Without this, Terser inlines the IIFE
    // and the minified variable name (e.g. 'r') becomes every route's name,
    // causing "Handler with name 'r' already exists" in production.
    /** @type {Route & Function} */
    const route = /** @type {any} */ (function (req, res, next) {
      const controller = this;
      controller.request = req;
      controller.response = res;
      route.dispatch(req.url, controller, next);
    });
    Object.defineProperty(route, 'name', { value: '', configurable: true });
    ...
    if (typeof fn === 'string') {
      route._actionStack.push(path, Object.assign(options, {
        template: fn
      }));
