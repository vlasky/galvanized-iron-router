...
UI.registerHelper('linkTo', new Blaze.Template('linkTo', function () {
  ...
  const data = _.extend({}, opts.data || DynamicTemplate.getParentDataContext(this));
  const route = Router.routes[routeName];
  ...
  const attrs = _.omit(opts, 'route', 'query', 'hash', 'data');
  attrs.href = path;
